...
function DatasetEditor({
  ...
  setDraft: Dispatch<SetStateAction<Partial<ChartDataset> | null>>;
  ...
  const patch = (fields: Partial<ChartDataset>) =>
    setDraft((prev) => {
      const prevDraft = (prev ?? {}) as Partial<ChartDataset>;
      const mergedBase: Partial<ChartDataset> = { ...prevDraft, ...fields };
      if (!fields.tableStyle) return mergedBase;
      return {
        ...mergedBase,
        tableStyle: {
          ...DEFAULT_TABLE_STYLE,
          ...((selected?.tableStyle ?? {}) as Partial<typeof DEFAULT_TABLE_STYLE>),
          ...((prevDraft.tableStyle ?? {}) as Partial<typeof DEFAULT_TABLE_STYLE>),
          ...(fields.tableStyle as Partial<typeof DEFAULT_TABLE_STYLE>),
        },
      };
    });
  ...
  const patchTableStyle = (field: keyof typeof DEFAULT_TABLE_STYLE, value: string) =>
    patch({ tableStyle: { [field]: value } as Partial<typeof DEFAULT_TABLE_STYLE> });
    ...
    <div>
      ...
      {tab === "chart" && (
        <div className="bg-white border border-[#E5E7EB] p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="md:col-span-2 border border-[#E5E7EB] rounded-xl px-4 py-3 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Table Color Palette</span>
              <button
                type="button"
                onClick={() => patch({ tableStyle: { ...DEFAULT_TABLE_STYLE } })}
                className="text-[10.5px] text-gray-500 hover:text-gray-900 underline"
              >
                Reset Palette
              </button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {([
                ...
                ["rowOddBg", "Row Odd BG"],
                ["rowEvenBg", "Row Even BG"],
                ["rowHoverBg", "Row Hover BG"],
                ["cellBorder", "Cell Border"],
                ["containerBg", "Container BG"],
                ["containerBorder", "Container Border"],
              ] as Array<[keyof typeof DEFAULT_TABLE_STYLE, string]>).map(([key, label]) => {
                const resolvedColor =
                  (effective.tableStyle?.[key] as string | undefined) ?? DEFAULT_TABLE_STYLE[key];
                return (
                <label key={key} className="flex items-center gap-2 border border-[#E5E7EB] rounded-lg px-2.5 py-2">
                  <input
                    type="color"
                    value={resolvedColor}
                    onChange={(e) => patchTableStyle(key, e.target.value)}
            ...
            <p className="text-[10.5px] text-gray-400">
              Warna tabel ini dipakai di halaman publik (Data Hub) dan hanya mengubah tampilan, bukan data.
            </p>
          </div>
        </div>
      )}
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB]">
      ...
      <div className="bg-white border-b border-[#E5E7EB] px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <span className="text-[12px] text-gray-500">Signed in as <span className="font-semibold text-gray-700">{adminUser}</span></span>
          <button
            onClick={() => {
          ...
          <button
            onClick={() => {
              if (confirm("Logout dari CMS admin?")) {
                void onLogout();
              }
            }}
            className="flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-semibold border border-gray-300 text-gray-700 hover:bg-gray-50"
          >
