...
export default function InteractiveChart({ dataset, height = 280 }: Props) {
  ...
  const yTickFormatter = (v: number) => {
    ...
    // Abbreviate large magnitudes (IDR-style): Jt / M (miliar) / T (triliun)
    if (abs >= 1_000_000_000_000) return formatNumberID(v / 1_000_000_000_000, 2) + "T";
    if (abs >= 1_000_000_000) return formatNumberID(v / 1_000_000_000, 1) + "M";
    if (abs >= 1_000_000) return formatNumberID(v / 1_000_000, 1) + "Jt";
