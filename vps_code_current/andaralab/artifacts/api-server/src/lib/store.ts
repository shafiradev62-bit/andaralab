...
export function normalizeDataset<T extends { columns?: string[]; rows?: Record<string, unknown>[]; yAxisMin?: number; yAxisMax?: number }>(ds: T): T {
  ...
  if (numericVals.length > 0) {
    ...
    // ── 3. Validate yAxisMin/yAxisMax (never clip real series values) ─────────
    // Min: reject only if it would hide points below the smallest value.
    // Max: reject only if below dataMax (would clip). Large headroom (triliunan+)
    // must be kept — the old "within 2×range" rule wrongly stripped valid scale-up.
...
export type ActivityAction = "create" | "update" | "delete" | "reset" | "bulk_create" | "login" | "logout";
