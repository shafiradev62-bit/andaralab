...
export default function ContactPage() {
  ...
  useEffect(() => {
    applyDocumentSeo({
      title: id ? "Kontak | AndaraLab" : "Contact | AndaraLab",
      description: id
        ? "Hubungi tim riset AndaraLab untuk kemitraan, pertanyaan riset, atau permintaan media."
  ...
  const subjects = id
    ? ["Kemitraan Riset", "Langganan Data", "Pertanyaan Media", "Pertanyaan Umum"]
    : ["Research Partnership", "Data Subscription", "Media Inquiry", "General Question"];
        ...
        <div className="max-w-[1200px] mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-10">
          <div className="md:col-span-1">
            <h2 className="text-[17px] font-semibold text-gray-900 mb-6">
              {id ? "Informasi Kontak" : "Contact Information"}
            </h2>
            <div className="space-y-5">
              {[
                { icon: Mail, label: "Email", value: "research@andaralab.id" },
                { icon: Phone, label: id ? "Telepon" : "Phone", value: "(+62) 812-1314-5883" },
                { icon: MapPin, label: id ? "Alamat" : "Address", value: "Jl. Pekayon 1 no 30C, Ragunan, Jakarta Selatan" },
              ].map((c) => (
            ...
            {submitted ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-[13.5px] text-gray-500">
                  {id ? "Kami akan menghubungi Anda dalam 1-2 hari kerja." : "We'll get back to you within 1-2 business days."}
                </p>
                <button onClick={() => setSubmitted(false)} className="mt-6 text-[13px] text-gray-900 font-medium hover:underline">
              ...
              <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="space-y-5">
                  ...
                  {(["name", "email", "organization"] as const).map((field) => (
                    <div key={field} className={field === "organization" ? "md:col-span-2" : ""}>
                      <label className="block text-[12px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
                        {field === "name" ? (id ? "Nama Lengkap" : "Full Name")
                          : field === "email" ? (id ? "Alamat Email" : "Email Address")
                          : (id ? "Organisasi" : "Organization")}{" "}
                      ...
                      <input
                        ...
                        placeholder={
                          field === "name" ? (id ? "Nama Anda" : "Your name")
                          : field === "email" ? "your@email.com"
                          : (id ? "Perusahaan atau institusi" : "Company or institution")
                        }
                      />
                    </div>
                  ))}
                ...
                <div>
                  <label className="block text-[12px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
                    {id ? "Subjek" : "Subject"} <span className="text-red-400">*</span>
                  </label>
                  <select
                    required
                    value={form.subject}
                    ...
                    className="w-full border border-[#E5E7EB] px-3 py-2.5 text-[13.5px] text-gray-900 focus:outline-none focus:border-gray-900 transition-colors bg-white"
                  >
                    <option value="">{id ? "Pilih subjek" : "Select subject"}</option>
                    {subjects.map((s) => <option key={s}>{s}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-[12px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
                    {id ? "Pesan" : "Message"} <span className="text-red-400">*</span>
                  </label>
                  <textarea
                    required
                    rows={5}
                    ...
                    placeholder={id ? "Jelaskan pertanyaan Anda..." : "Describe your inquiry..."}
                  />
                </div>
                <button
                  type="submit"
