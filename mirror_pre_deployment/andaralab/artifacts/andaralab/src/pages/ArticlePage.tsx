  ...
  if (!post) {
    ...
    return (
      <div className="max-w-[1200px] mx-auto px-6 py-24 text-center">
        ...
        <h1 className="text-[24px] font-semibold text-gray-900 mb-3">{t("meta_article_not_found_title")}</h1>
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2">
            ...
            {post.image && (post.imagePosition === "top" || !post.imagePosition) && (
            ...
            <div className="prose max-w-none">
              {post.image && post.imagePosition === "left" && (
                <div className="float-left mr-6 mb-4 w-1/2 md:w-1/3 border border-[#E5E7EB] overflow-hidden">
                  <img src={post.image} alt={post.title} className="w-full h-auto object-cover" />
                </div>
              )}
              {post.image && post.imagePosition === "right" && (
                <div className="float-right ml-6 mb-4 w-1/2 md:w-1/3 border border-[#E5E7EB] overflow-hidden">
                  <img src={post.image} alt={post.title} className="w-full h-auto object-cover" />
                </div>
              )}
              {bodyParagraphs.map((para, i) => {
                if (typeof para === "string" && para.startsWith("[IMG: ") && para.endsWith("]")) {
                  return (
                    <div key={i} className="my-8 border border-[#E5E7EB] rounded-lg overflow-hidden shadow-sm">
                      <img src={para.slice(6, -1)} alt="Article body content" className="w-full h-auto object-cover" />
                    </div>
                  );
                }
                return (
            ...
            {post.image && post.imagePosition === "bottom" && (
              <div className="mt-8 border border-[#E5E7EB] overflow-hidden">
                <img src={post.image} alt={post.title} className="w-full h-[400px] object-cover" />
              </div>
            )}
            ...
            {post.images && post.images.length > 0 && (
              <div className="mt-10">
                <h3 className="text-[14px] font-bold text-gray-900 mb-4">{locale === "id" ? "Galeri Gambar" : "Image Gallery"}</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {post.images.map((img, idx) => (
                    <div key={idx} className="relative aspect-video border border-[#E5E7EB] overflow-hidden">
                      <img src={img} alt={`${post.title} - gallery ${idx + 1}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
                    </div>
                  ))}
                </div>
              </div>
