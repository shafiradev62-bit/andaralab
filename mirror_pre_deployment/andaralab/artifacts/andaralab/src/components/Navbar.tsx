...
function isFinancialSection(section?: string) {
  return (section || "").toLowerCase().includes("financial");
}

function isEnergyRelated(p: { title?: string; navLabel?: string; section?: string; slug?: string }) {
  const text = [p.title, p.navLabel, p.section, p.slug].join(" ").toLowerCase();
  return text.includes("energy") || text.includes("coal") || text.includes("oil") || text.includes("gas");
}
...
export default function Navbar({ dark = false }: { dark?: boolean }) {
  ...
  // Blog children — only Economics 101 and Market Pulse (Lab Notes removed per product decision)
  const blogNavChildren: NavChild[] = [
  ...
  const navItems: NavItem[] = useMemo(() => {
    ...
    const sectoralChildren = pub
      .filter((p) => isSectoralSection(p.section) && !isEnergyRelated(p))
      .sort((a, b) => (a.navLabel || a.title).localeCompare(b.navLabel || b.title))
      .map((p) => ({ label: p.navLabel || p.title, href: p.slug }));

    const customSectionMap = new Map<string, NavChild[]>();
    pub
      .filter((p) => p.section && !FIXED_SECTIONS.has(p.section) && p.slug !== "/" && !isFinancialSection(p.section))
      .forEach((p) => {
        const sec = p.section!;
        const label = p.navLabel || p.title;
        // Skip if child label matches section name (prevents duplicate)
        if (label === sec) return;
        if (!customSectionMap.has(sec)) customSectionMap.set(sec, []);
        customSectionMap.get(sec)!.push({ label, href: p.slug });
      });
    ...
    return [
      ...
      ...customNavItems,
      {
        ...
        // LLM Model Comparison removed per product decision
      },
  ...
  return (
    <header className={`${dark ? "bg-black/40 backdrop-blur-sm border-b border-white/10" : "bg-white border-b border-[#E5E7EB]"}`}>
      <div className="max-w-[1200px] mx-auto px-6 flex items-center h-14">
        <LLink href="/" className="flex items-center gap-2 mr-6 flex-shrink-0">
