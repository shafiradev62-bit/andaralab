...
export default function MarketTicker({ dark = false }: { dark?: boolean }) {
  ...
  return (
    <div className={`relative h-8 flex items-center ${dark ? "bg-black/50 border-b border-white/10" : "bg-transparent border-b border-[#E5E7EB]"}`}>
      <div className={`flex-shrink-0 flex items-center gap-2 px-3 border-r h-full z-10 ${dark ? "border-white/10" : "border-[#E5E7EB]"} ${dark ? "bg-transparent" : "bg-transparent"}`}>
