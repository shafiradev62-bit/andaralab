...
function getColumnLabel(dataset: ChartDataset, colKey: string, locale?: string): string {
  if (dataset.columnNames) {
    const localeKey = locale === "id" ? "id" : "en";
    const localeNames = dataset.columnNames[localeKey] ?? dataset.columnNames.en ?? dataset.columnNames.id;
...
function CustomTooltip({ active, payload, label, dataset, columnNames }: any) {
  ...
  // For combo charts, determine which axis each key belongs to
  const barKeys = new Set(dataset.comboConfig?.barColumns ?? []);
  const lineKeys = new Set(dataset.comboConfig?.lineColumns ?? []);
  ...
  return (
    <div className="bg-white border border-[#E5E7EB] shadow-lg px-4 py-3 rounded-sm min-w-[180px] z-50">
      ...
      {payload.map((p: any, i: number) => {
        ...
        // Determine unit type and unit label per series for combo charts
        let unitType = p.unitType ?? dataset.unitType;
        let seriesUnit = effectiveUnit;
        if (dataset.chartType === "combo" && dataset.comboConfig) {
          if (barKeys.has(p.dataKey)) {
            unitType = dataset.comboConfig.leftUnitType ?? unitType;
            seriesUnit = (dataset.comboConfig.leftUnit || effectiveUnit).trim();
          } else if (lineKeys.has(p.dataKey)) {
            unitType = dataset.comboConfig.rightUnitType ?? unitType;
            seriesUnit = (dataset.comboConfig.rightUnit || "").trim();
          }
        }
        ...
        return (
          <div key={i} className="flex items-center justify-between gap-4 py-1">
            ...
            <span className="text-[12px] font-bold text-gray-900">
              {typeof p.value === "number" ? formatValue(p.value, unitType, seriesUnit) : p.value}
            </span>
          </div>
        );
      })}
...
export default function InteractiveChart({ dataset, height = 280 }: Props) {
  ...
  for (const col of dataset.columns) {
    columnNameMap[col] = getColumnLabel(dataset, col, locale);
  }
  ...
  const yAxisDomain = useMemo(() => {
    ...
    // EXCEPTION: if the ratio between max and min is very large (>10x),
    // don't force 0 baseline — it would squish smaller series to invisible
    const ratio = actualMax / (actualMin || 1);
    if (actualMin >= 0 && min < 0 && ratio <= 10) min = 0;
  ...
  // When multiple series have very different magnitudes (ratio > 50x),
  // use log scale so all series are visible
  const useLogScale = useMemo(() => {
    if (dataKeys.length < 2) return false;
    // Get max value per series
    const seriesMaxes = dataKeys.map(key =>
      Math.max(...data.map(r => {
        const v = r[key];
        return typeof v === "number" ? v : parseFloat(String(v));
      }).filter(v => !isNaN(v) && v > 0))
    ).filter(v => isFinite(v) && v > 0);
    if (seriesMaxes.length < 2) return false;
    const overallMax = Math.max(...seriesMaxes);
    const overallMin = Math.min(...seriesMaxes);
    // If largest series is 50x bigger than smallest, use log scale
    return overallMax / overallMin > 50;
  }, [data, dataKeys]);
        ...
        <ResponsiveContainer width="100%" height={height + 50}>
          <AreaChart {...commonProps}>
            ...
            <YAxis
              {...axisStyle}
              width={75}
              domain={useLogScale ? ["auto", "auto"] : yAxisDomain}
  ...
  if (dataset.chartType === "combo") {
    ...
    const leftLabel = dataset.comboConfig?.leftLabel || dataset.comboConfig?.leftUnit || effectiveUnit || "(Indeks)";
    const rightLabel = dataset.comboConfig?.rightLabel ?? dataset.comboConfig?.rightUnit ?? "(%)";

    // Build right-axis tick formatter based on rightUnitType or rightUnit
    const rightUnitType = dataset.comboConfig?.rightUnitType;
    const rightUnit = (dataset.comboConfig?.rightUnit || "").trim();
    const rightTickFormatter = (v: number): string => {
      if (!Number.isFinite(v)) return String(v);
      if (rightUnitType === "percent") return `${formatNumberID(v, 1)}%`;
      if (rightUnitType === "currency_usd") {
        const abs = Math.abs(v);
        if (abs >= 1_000_000_000) return `$${formatNumberID(v / 1_000_000_000, 1)}B`;
        if (abs >= 1_000_000) return `$${formatNumberID(v / 1_000_000, 1)}M`;
        return `$${formatNumberID(v, 0)}`;
      }
      if (rightUnitType === "currency_idr") {
        const abs = Math.abs(v);
        if (abs >= 1_000_000_000) return `${formatNumberID(v / 1_000_000_000, 1)}T`;
        if (abs >= 1_000_000) return `${formatNumberID(v / 1_000_000, 1)}M`;
        return formatNumberID(v, 0);
      }
      // custom or number: append unit label if provided
      const abs = Math.abs(v);
      const formatted = abs >= 1_000_000_000
        ? `${formatNumberID(v / 1_000_000_000, 1)}B`
        : abs >= 1_000_000
        ? `${formatNumberID(v / 1_000_000, 1)}M`
        : formatNumberID(v, 1);
      return rightUnit ? `${formatted} ${rightUnit}` : `${formatNumberID(v, 1)}%`;
    };

    // Left-axis tick formatter based on leftUnitType
    const leftUnitType = dataset.comboConfig?.leftUnitType;
    const leftUnit = (dataset.comboConfig?.leftUnit || "").trim();
    const leftTickFormatter = (v: number): string => {
      if (!Number.isFinite(v)) return String(v);
      if (leftUnitType === "percent") return `${formatNumberID(v, 1)}%`;
      if (leftUnitType === "currency_usd") {
        const abs = Math.abs(v);
        if (abs >= 1_000_000_000) return `$${formatNumberID(v / 1_000_000_000, 1)}B`;
        if (abs >= 1_000_000) return `$${formatNumberID(v / 1_000_000, 1)}M`;
        return `$${formatNumberID(v, 0)}`;
      }
      if (leftUnitType === "currency_idr") {
        const abs = Math.abs(v);
        if (abs >= 1_000_000_000) return `${formatNumberID(v / 1_000_000_000, 1)}T`;
        if (abs >= 1_000_000) return `${formatNumberID(v / 1_000_000, 1)}M`;
        return formatNumberID(v, 0);
      }
      // Fallback to default yTickFormatter
      ...
      <div className="space-y-0 w-full flex flex-col min-h-0" {...zoomProps}>
        ...
        <ResponsiveContainer width="100%" height={height + 50}>
          <ComposedChart {...commonProps}>
            <CartesianGrid strokeDasharray="3 3" stroke="#F3F4F6" vertical={false} />
            {xAxis}
            <YAxis yAxisId="left" {...axisStyle} width={75} domain={comboLeftDomain ?? yAxisDomain} tickFormatter={leftUnitType ? leftTickFormatter : yTickFormatter}>
              <Label value={leftLabel} offset={15} position="top" style={{ fontSize: 10, fill: '#9CA3AF', fontWeight: 'bold' }} />
            </YAxis>
            <YAxis yAxisId="right" orientation="right" {...axisStyle} width={50} domain={comboRightDomain ?? ["auto", "auto"]} tickFormatter={rightTickFormatter}>
