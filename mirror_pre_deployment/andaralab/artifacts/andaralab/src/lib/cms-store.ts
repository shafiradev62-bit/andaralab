...
export interface ChartDataset {
  ...
  comboConfig?: {
    ...
    leftUnitType?: DataUnitType; // unit type for left axis (e.g. "currency_usd")
    rightUnitType?: DataUnitType; // unit type for right axis (e.g. "percent")
    leftUnit?: string;      // raw unit label for left axis
    rightUnit?: string;     // raw unit label for right axis
  };
  // Donut chart config
  donutConfig?: {
...
const ALLOW_API_FALLBACK = import.meta.env.VITE_ALLOW_API_FALLBACK === "true";
...
export async function fetchPages(filter?: { locale?: string; status?: string; section?: string }): Promise<Page[]> {
  try {
    ...
    // Always replace localStorage with fresh API data — never merge with stale cache.
    // This ensures all clients (including those with old localStorage) always see VPS data.
    if (!filter?.locale && !filter?.status && !filter?.section) {
      // Full fetch — replace everything
      savePagesToStorage(res.data);
    } else {
      // Filtered fetch — merge carefully: replace matching locale/status, keep others
      const existing = getSeedPages().filter((p) => {
        if (filter?.locale && p.locale === filter.locale) return false;
        if (!filter?.locale && filter?.status && p.status === filter.status) return false;
        return true;
      });
      savePagesToStorage([...existing, ...res.data]);
    }
    return res.data;
  } catch (error) {
    if (!ALLOW_API_FALLBACK) throw error;
...
export async function fetchPageBySlug(slug: string, locale?: string): Promise<Page> {
  ...
  if (apiFailed) {
    ...
    // Strict locale: do not fall back to a different locale
    if (pick) return pick;
