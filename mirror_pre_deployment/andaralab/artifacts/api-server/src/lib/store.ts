...
import { uploadToS3 } from "./s3-sync.js";
...
/** Exported so GET /api/datasets can return the same X-axis fixes as create/update. */
export function normalizeDataset<T extends { columns?: string[]; rows?: Record<string, unknown>[]; yAxisMin?: number; yAxisMax?: number }>(ds: T): T {
  ...
  // ── Fix X axis labels (fill missing years) ──────────────────────────────────
  const xkey = cols[0];
  ...
  // ── IMPORTANT: yAxisMin/yAxisMax are NEVER stripped here ────────────────────
  // These values are set intentionally by the admin. Removing them silently
  // causes data loss. If the chart looks wrong, the admin should fix it manually.
...
function readJson<T>(filename: string, fallback: T): T {
  ...
  try {
    if (fs.existsSync(filepath)) {
      ...
      if (!raw.trim()) {
        console.warn(`[store] ⚠ ${filepath} is empty, using seed data`);
        return fallback;
      }
      const parsed = JSON.parse(raw) as T;
      console.log(`[store] ✓ Loaded ${filename} from disk (${Array.isArray(parsed) ? parsed.length + " items" : "object"})`);
      return parsed;
    } else {
      console.warn(`[store] ⚠ ${filepath} not found, using seed data`);
    }
  } catch (err) {
    console.error(`[store] ✗ Failed to read/parse ${filepath}:`, err);
    ...
    if (fs.existsSync(backupPath)) {
      try {
        ...
        const parsed = JSON.parse(raw) as T;
        console.warn(`[store] ⚠ Recovered ${filename} from backup (${Array.isArray(parsed) ? parsed.length + " items" : "object"})`);
        // Restore the primary file from backup
        fs.copyFileSync(backupPath, filepath);
        return parsed;
      } catch (backupErr) {
        console.error(`[store] ✗ Backup recovery also failed:`, backupErr);
      }
    }
    console.warn(`[store] Using fallback seed data for ${filename}`);
  }
  return fallback;
}
...
function writeJson<T>(filename: string, data: T): void {
  ...
  try {
    ...
    console.log(`[store] ✓ Saved ${filename} (${Array.isArray(data) ? (data as unknown[]).length + " items" : "object"})`);

    // ── S3 backup: fire-and-forget, never blocks the response ────────────────
    uploadToS3(filename, jsonStr).catch((err) =>
      console.error(`[s3] Background upload failed for ${filename}:`, err)
    );
  } catch (e) {
...
class PersistentAnalisisDeskriptifStore implements AnalisisDeskriptifStore {
  ...
  bulkCreate(items: Omit<AnalisisDeskriptifRecord, "id" | "createdAt" | "updatedAt">[]): AnalisisDeskriptifRecord[] {
    const records: AnalisisDeskriptifRecord[] = [];
    for (const data of items) {
      const id = `analisis-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
      const record: AnalisisDeskriptifRecord = { ...cloneDeep(data), id, createdAt: now(), updatedAt: now() };
      this.records.set(id, record);
...
// ⚠️ CRITICAL: Use FIXED dates for seed data, NOT dynamic dates!
// Dynamic dates cause data loss on every server restart.
// Use dates far in the future so they don't expire quickly.
const SEED_CALENDAR_EVENTS: CalendarEvent[] = [
  {
    id: "cal-1",
    date: "2026-05-01",  // FIXED DATE - do not use new Date()
    time: "13:30",
    countryCode: "US",
...
export type ActivityAction = "create" | "update" | "delete" | "reset" | "bulk_create" | "login";
export type ActivityResource = "dataset" | "page" | "post" | "analisis" | "featured_insights" | "exchange_rate" | "calendar_event" | "calendar_config" | "auth";
...
export interface ActivityLogEntry {
  ...
  /** Human-readable diff of changed fields */
  diff?: string;
}

export interface ActivityLogStore {
  log(entry: Omit<ActivityLogEntry, "id" | "timestamp">): void;
  list(limit?: number, filter?: { resource?: ActivityResource; action?: ActivityAction }): ActivityLogEntry[];
  clear(): void;
  /** Export all entries as JSON string (for backup/download) */
  export(): string;
}
...
class PersistentActivityLogStore implements ActivityLogStore {
  ...
  private readonly MAX = 2000; // increased from 500
  ...
  log(entry: Omit<ActivityLogEntry, "id" | "timestamp">): void {
    ...
    this.save();
    // Also print to console so it shows in server logs
    console.log(`[audit] ${record.timestamp} | ${record.action.toUpperCase()} ${record.resource} | id=${record.resourceId ?? "-"} | "${record.resourceTitle ?? ""}" | ip=${record.ip} | ${record.detail ?? ""}`);
  }

  list(limit = 100, filter?: { resource?: ActivityResource; action?: ActivityAction }): ActivityLogEntry[] {
    ...
    return entries.slice(0, Math.min(limit, this.MAX));
  }
