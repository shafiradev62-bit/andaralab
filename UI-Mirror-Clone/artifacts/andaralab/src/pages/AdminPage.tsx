...
import {
  ...
  useResetAnalisis,
  useFeaturedInsights,
  useUpdateFeaturedInsights,
  useResetFeaturedInsights,
  useExchangeRates,
  useCreateExchangeRate,
  useUpdateExchangeRate,
  useDeleteExchangeRate,
  useResetExchangeRates,
  useCalendarEvents,
  useCalendarConfig,
  useCreateCalendarEvent,
  useUpdateCalendarEvent,
  useDeleteCalendarEvent,
  useResetCalendarEvents,
  ...
  type Page,
  type BlogPost,
  type AnalisisDeskriptif,
  type AnalysisSection,
  type AnalysisWidget,
  type AnalysisMetric,
  type AnalysisWidgetType,
  type FeaturedInsight,
  type FeaturedInsightsConfig,
  type DataUnitType,
  type ExchangeRate,
  type CalendarEvent,
  type CalendarConfig,
...
const CATEGORIES = ["Macro Foundations", "Sectoral Intelligence"];
...
const SECTIONS = ["root", "Macro Foundations", "Sectoral Intelligence"];
...
function StatusBadge({ status }: { status: string }) {
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10.5px] font-semibold uppercase tracking-wide ${
        status === "published"
...
function LocaleBadge({ locale }: { locale: string }) {
  return (
    <span
      className={`text-[10.5px] font-semibold uppercase px-1.5 py-0.5 ${
...
function DatasetEditor({
  ...
  selected: ChartDataset | null;
  draft: Partial<ChartDataset> | null;
  ...
  const [tab, setTab] = useState<
    "meta" | "chart" | "columns" | "data" | "preview"
  >("meta");
  const [showSavedMsg, setShowSavedMsg] = useState(false);
  const effective =
    draft !== null ? ({ ...selected, ...draft } as ChartDataset) : selected;
  ...
  const addColumn = () => {
    ...
    patch({
      columns: [...effective.columns, name],
      rows: effective.rows.map((r) => ({ ...r, [name]: 0 })),
      colors: newColors,
    });
  };
  ...
  const removeColumn = (col: string) => {
    ...
    const newColors = (effective.colors ?? []).filter(
      (_, i) => i !== colIdx - 1,
    );
    patch({
      columns: effective.columns.filter((c) => c !== col),
      rows: effective.rows.map((r) => {
        const nr = { ...r };
        delete nr[col];
        return nr;
      }),
  ...
  const renameColumn = (oldName: string, newName: string) => {
    patch({
      columns: effective.columns.map((c) => (c === oldName ? newName : c)),
      rows: effective.rows.map((r) => {
        const nr = { ...r };
  ...
  const addRow = () => {
    ...
    effective.columns.forEach((c, i) => {
      newRow[c] = i === 0 ? "New Row" : 0;
    });
    patch({ rows: [...effective.rows, newRow] });
  };

  const removeRow = (idx: number) =>
    patch({ rows: effective.rows.filter((_, i) => i !== idx) });

  const updateCell = (rowIdx: number, col: string, val: string) => {
    patch({
      rows: effective.rows.map((r, i) => {
        if (i !== rowIdx) return r;
        const colIdx = effective.columns.indexOf(col);
        return {
          ...r,
          [col]: colIdx > 0 ? (isNaN(Number(val)) ? val : Number(val)) : val,
        };
      }),
    });
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <button
            onClick={onBack}
            className="flex items-center gap-1.5 text-[13px] text-gray-500 hover:text-gray-900 font-medium"
          >
      ...
      {tab === "meta" && (
        <div className="bg-white border border-[#E5E7EB] p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <select
              value={effective.unitType ?? "number"}
              onChange={(e) =>
                patch({ unitType: e.target.value as DataUnitType })
              }
            ...
            >
              {(
                Object.entries(UNIT_TYPE_LABELS) as [DataUnitType, string][]
              ).map(([val, label]) => (
          ...
          <div className="animate-in fade-in slide-in-from-top-1 duration-200 md:col-span-2">
            ...
            />
            <p className="text-[10.5px] text-gray-400 mt-1">
              This label will be used for Axis labels, Tooltips, and Data
              Tables.
            </p>
          </div>
          ...
          <div className="md:col-span-2 border border-amber-100 bg-amber-50 rounded-xl px-4 py-3 flex items-center justify-between">
            <div>
              <span className="text-[12px] font-bold text-amber-800 uppercase tracking-wide block mb-0.5">
                Time Series Ordering
              </span>
              <p className="text-[11px] text-amber-600">
                Preserve exactly how data is entered in the table below (Top
                &rarr; Bottom).
              </p>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-medium text-amber-700">
                {effective.preserveInputOrder ? "Input Order" : "Chrono Order"}
              </span>
              <button
                type="button"
                onClick={() =>
                  patch({ preserveInputOrder: !effective.preserveInputOrder })
                }
                className={`w-10 h-5 rounded-full transition-colors relative ${effective.preserveInputOrder ? "bg-amber-500" : "bg-gray-300"}`}
              >
                <div
                  className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${effective.preserveInputOrder ? "left-6" : "left-1"}`}
                />
              </button>
            </div>
          </div>
          ...
          <div className="md:col-span-2 border border-gray-200 rounded-xl px-4 py-3 space-y-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              {(effective.yAxisMin !== undefined ||
                effective.yAxisMax !== undefined) && (
                <button
                  onClick={() => {
                    patch({ yAxisMin: undefined, yAxisMax: undefined });
                  }}
            ...
            <p className="text-[10.5px] text-gray-400">
              Leave empty for auto-scale. Example: set Min = 90 to start Y-axis
              at 90 instead of 0.
            </p>
          </div>
            ...
            <div className="flex flex-col gap-2">
              {usedCustomCategories.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  <span className="text-[10.5px] text-gray-400 w-full">
                    Previously used:
                  </span>
          ...
          <div>
            ...
            <p className="text-[10.5px] text-gray-400 mt-1">
              Used for granular filtering on sub-menus.
            </p>
          </div>
          <div>
            <label className="block text-[11.5px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
            ...
            <p className="text-[11px] text-gray-400 mt-2">
              {effective.chartType === "combo"
                ? "Bar + Line dual-axis chart. Configure below."
                : effective.chartType === "donut"
                  ? "Pie chart with center hole. Configure below."
      ...
      {tab === "chart" && (
        <div className="bg-white border border-[#E5E7EB] p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="md:col-span-2">
            <div className="flex items-center gap-2 mb-4 pb-4 border-b border-[#E5E7EB]">
              ...
              <span className="text-[12.5px] text-gray-600">
                These labels override the chart display title and axis labels.
                Both English and Indonesian versions can be set.
              </span>
            </div>
          </div>
          ...
          <div className="md:col-span-2">
            <label className="block text-[11.5px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
              Chart Title (Bahasa Indonesia)
            </label>
            <input
              type="text"
              value={effective.chartTitleId ?? ""}
          ...
          <div>
            <label className="block text-[11.5px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
              X-Axis Label (Bahasa Indonesia)
            </label>
            <input
              type="text"
              value={effective.xAxisLabelId ?? ""}
          ...
          <div>
            <label className="block text-[11.5px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
              Y-Axis Label (Bahasa Indonesia)
            </label>
            <input
              type="text"
              value={effective.yAxisLabelId ?? ""}
            ...
            <div className="md:col-span-2 border border-gray-200 rounded-xl p-5 space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {effective.columns.slice(1).map((col) => {
                    const isBar =
                      effective.comboConfig?.barColumns?.includes(col);
                    return (
                      <button
                        key={col}
                        onClick={() => {
                          const prev =
                            effective.comboConfig?.barColumns ??
                            effective.columns.slice(1);
                          const next = isBar
                            ? prev.filter((c) => c !== col)
                            : [...prev.filter((c) => c !== col), col];
                          patch({
                            comboConfig: {
                              ...effective.comboConfig,
                              barColumns: next,
                              lineColumns: effective.columns
                                .slice(1)
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {effective.columns.slice(1).map((col) => {
                    const isLine =
                      effective.comboConfig?.lineColumns?.includes(col);
                    return (
                      <button
                        key={col}
                        onClick={() => {
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11.5px] font-semibold text-gray-600 uppercase tracking-wide mb-1">
                    Left Axis Label
                  </label>
                  <input
                    type="text"
                    value={effective.comboConfig?.leftLabel ?? ""}
                    onChange={(e) =>
                      patch({
                        comboConfig: {
                          ...effective.comboConfig,
                          leftLabel: e.target.value,
                          barColumns: effective.columns.slice(1),
                          lineColumns: effective.comboConfig?.lineColumns ?? [],
                        },
                      })
                    }
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={effective.comboConfig?.rightLabel ?? ""}
                    onChange={(e) =>
                      patch({
                        comboConfig: {
                          ...effective.comboConfig,
              ...
              <div className="border-t border-gray-200 pt-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {(effective.comboConfig?.leftAxisMin !== undefined ||
                    effective.comboConfig?.leftAxisMax !== undefined ||
                    effective.comboConfig?.rightAxisMin !== undefined ||
                    effective.comboConfig?.rightAxisMax !== undefined) && (
                    <button
                      onClick={() =>
                        patch({
                          comboConfig: {
                            ...effective.comboConfig,
                            leftAxisMin: undefined,
                            leftAxisMax: undefined,
                            rightAxisMin: undefined,
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10.5px] font-semibold text-gray-600 uppercase tracking-wide mb-1">
                      Left Axis Min (Index)
                    </label>
                    <input
                      type="number"
                      ...
                      onChange={(e) =>
                        patch({
                          comboConfig: {
                            barColumns: effective.comboConfig?.barColumns ?? [],
                            lineColumns:
                              effective.comboConfig?.lineColumns ?? [],
                            ...effective.comboConfig,
                            leftAxisMin:
                              e.target.value === ""
                                ? undefined
                ...
                <p className="text-[10px] text-gray-400">
                  Kosongkan untuk auto-scale. Left = sumbu kiri (Index), Right =
                  sumbu kanan (%).
                </p>
              </div>
              ...
              <div className="border-t border-gray-200 pt-4 space-y-3">
                <div className="flex items-center gap-2">
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        patch({
                          donutConfig: {
                            labelColumn:
                              effective.donutConfig?.labelColumn ??
                              effective.columns[0],
                            valueColumn:
                              effective.donutConfig?.valueColumn ?? valKey,
                        ...
                        patch({
                          donutConfig: {
                            labelColumn:
                              ...
                              effective.columns[0],
                            valueColumn:
                              effective.donutConfig?.valueColumn ??
                              effective.columns[1] ??
                              "",
                            ...effective.donutConfig,
                  ...
                  />
                  <label
                    htmlFor="enableBreakdown"
                    className="text-[12px] font-semibold text-gray-700"
                  >
                    Add GDP Composition Breakdown Below Chart
                  </label>
                </div>
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <select
                          value={
                            effective.donutConfig?.breakdownLabelCol ??
                            effective.columns[0]
                          }
                          onChange={(e) =>
                            patch({
                      ...
                      <div>
                        ...
                        <select
                          value={
                            effective.donutConfig?.breakdownValueCol ??
                            effective.columns[1] ??
                            ""
                          }
                          onChange={(e) =>
                          ...
                          className="w-full border border-gray-200 px-2 py-1.5 text-[12px] rounded focus:outline-none focus:border-blue-400 bg-white"
                        >
                          {effective.columns.slice(1).map((c) => (
                            <option key={c} value={c}>
                              {c}
                            </option>
                          ))}
                    ...
                    <div className="flex items-center gap-3">
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            type="color"
                            value={
                              effective.donutConfig?.breakdownBorderColor ??
                              "#E5E7EB"
                            }
                            onChange={(e) =>
                              patch({
                                donutConfig: {
                                  ...effective.donutConfig,
                                  breakdownBorderColor: e.target.value,
                                },
                              })
                            }
                      ...
                      <div className="text-[10.5px] text-gray-400 mt-4">
                        Border & top-line color for the breakdown section
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="md:col-span-2 border border-purple-200 bg-purple-50 rounded-xl p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    value={
                      effective.donutConfig?.labelColumn ?? effective.columns[0]
                    }
                    onChange={(e) =>
                      patch({
                        donutConfig: {
              ...
              <div className="flex items-center gap-5">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <label
                    htmlFor="donutShowPct"
                    className="text-[12px] font-medium text-gray-600"
                  >
                    Show % instead of raw value
                  </label>
                </div>
                <div className="flex items-center gap-2">
                  <label className="text-[11.5px] font-medium text-gray-500">
                    Inner Radius:
                  </label>
                  <input
                    type="range"
                    min="0"
                    max="90"
                    value={effective.donutConfig?.innerRadiusPercent ?? 60}
                    onChange={(e) =>
                      patch({
                        donutConfig: {
                          ...effective.donutConfig,
                          innerRadiusPercent: Number(e.target.value),
                        },
                      })
                    }
                  ...
                  <span className="text-[12px] text-gray-600">
                    {effective.donutConfig?.innerRadiusPercent ?? 60}%
                  </span>
                </div>
              </div>
              ...
              <div className="border-t border-purple-200 pt-4 space-y-3">
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id="enableDonutBreakdown"
                    checked={!!effective.donutConfig?.breakdownRows?.length}
                    onChange={(e) => {
                      if (e.target.checked) {
                        const labelKey =
                          Object.keys(effective.rows[0] ?? {}).find(
                            (k) => typeof effective.rows[0][k] === "string",
                          ) ?? effective.columns[0];
                        const valKey =
                          effective.donutConfig?.valueColumn ??
                          effective.columns[1] ??
                  ...
                  />
                  <label
                    htmlFor="enableDonutBreakdown"
                    className="text-[12px] font-semibold text-purple-700"
                  >
                    Add Breakdown Table Below Donut
                  </label>
                </div>
                {effective.donutConfig?.breakdownRows?.length > 0 && (
                  <div className="grid grid-cols-2 gap-3">
          ...
          <div className="md:col-span-2 border border-gray-200 rounded-xl px-4 py-3 space-y-3">
            ...
            <p className="text-[10.5px] text-gray-400">
              Kosongkan untuk auto-scale. Contoh: Min = 0 agar sumbu Y mulai
              dari 0.
            </p>
          </div>
        ...
        <div className="bg-white border border-[#E5E7EB] p-6">
          <div className="mb-5 pb-4 border-b border-[#E5E7EB]">
            <p className="text-[12.5px] text-gray-600">
              Set display names for columns per language. Leave blank to use the
              raw column key.
            </p>
          </div>
          {effective.columns.map((col, colIdx) => (
            <div key={col} className="mb-5 p-4 border border-[#E5E7EB]">
              <div className="text-[11px] font-bold text-gray-400 uppercase tracking-wide mb-3">
                Column {colIdx + 1}:{" "}
                <span className="text-gray-700 font-mono">{col}</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const currentEn = effective.columnNames?.en ?? [
                        ...effective.columns,
                      ];
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const currentId = effective.columnNames?.id ?? [
                        ...effective.columns,
                      ];
        ...
        <div className="bg-white border border-[#E5E7EB] p-6">
          <div className="flex items-center justify-between mb-4">
            <p className="text-[13px] text-gray-600">
              First column = X-axis labels. Remaining columns = data series.
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => {
                  ...
                  if (
                    xVal.toLowerCase().includes("q4") ||
                    xVal.toLowerCase().includes("iv")
                  )
                    nextYear++;
                  ...
                  const quarters = ["Q1", "Q2", "Q3", "Q4"].map((q) => ({
                    ...
                    ...Object.fromEntries(
                      effective.columns.slice(1).map((c) => [c, 0]),
                    ),
                  }));
              ...
              <button
                onClick={addColumn}
                className="flex items-center gap-1 text-[12px] font-medium text-gray-700 border border-gray-300 px-3 py-1.5 hover:bg-gray-50"
              >
                <Plus className="w-3.5 h-3.5" /> Add Series
              </button>
              <button
                onClick={addRow}
                className="flex items-center gap-1 text-[12px] font-medium text-white bg-gray-900 px-3 py-1.5 hover:bg-gray-700"
              >
            ...
            {effective.columns.slice(1).length > 0 && (
              <div className="mb-3 pb-3 border-b border-[#E5E7EB]">
                ...
                <div className="flex flex-wrap gap-3">
                  {effective.columns.slice(1).map((col, i) => {
                    const currentColor =
                      (effective.colors ?? [])[i] ?? "#1a3a5c";
                    return (
                      <div key={col} className="flex items-center gap-1.5">
                        <span className="text-[11px] text-gray-600 font-medium w-20 truncate">
                          {col}
                        </span>
                        <div className="flex items-center gap-0.5">
                          ...
                          <input
                            ...
                            value={currentColor}
                            onChange={(e) =>
                              updateSeriesColor(i, e.target.value)
                            }
              ...
              <thead>
                <tr>
                  {effective.columns.map((col, i) => (
                    <th
                      key={col}
                      className="border border-[#E5E7EB] bg-gray-50 px-2 py-1.5 text-left min-w-[100px]"
                    >
                ...
                {effective.rows.map((row, ri) => (
                  <tr key={ri} className="hover:bg-gray-50">
                    {effective.columns.map((col) => (
                      <td
                        key={col}
                        className="border border-[#F3F4F6] px-2 py-1"
                      >
                        <input
                          value={row[col] ?? ""}
      ...
      {tab === "preview" && (
        <div className="bg-white border border-[#E5E7EB] p-6">
              ...
              {effective.columns.slice(1).map((col, i) => {
                ...
                return (
                  <div key={col} className="flex items-center gap-2">
                    <span className="text-[12px] font-medium text-gray-700 w-28 truncate">
                      {col}
                    </span>
                    <div className="flex items-center gap-1">
                      {CHART_PALETTE.map((c) => (
          ...
          <p className="text-[11px] text-gray-400 mt-4">
            ...
            {(effective as any).chartTitle &&
              ` · Chart title: ${(effective as any).chartTitle}`}
          </p>
        </div>
      )}
...
function PageEditor({
  ...
  const [draft, setDraft] = useState<Partial<Page>>(() =>
    page ? { ...page } : { ...newPage(defaultLocaleForNew) },
  );
  ...
  const doSave = (onSuccess?: () => void) => {
    ...
    if (!statusOk) {
      setSaveError(
        "Pilih status dulu: Draft (hidden) atau Published (live), baru klik Save.",
      );
      return;
    }
    ...
    if (isNew) {
      createMut.mutate(
        payload as Omit<Page, "id" | "createdAt" | "updatedAt">,
        {
  ...
  const viewHref = savedSlug
    ? savedSlug.startsWith("/")
      ? savedSlug
      : `/${savedSlug}`
    : "/";
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <LocaleBadge
            locale={(draft.locale ?? page?.locale ?? "en") as "en" | "id"}
          />
        </div>
        {/* Auto-save status indicator */}
        <div className="flex items-center gap-2">
      ...
      <div className="bg-white border border-[#E5E7EB] p-6 grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
        ...
        <div>
          ...
          />
          <p className="text-[10.5px] text-gray-400 mt-1">
            e.g. /macro/macro-outlooks or /about
          </p>
        </div>
        ...
        <div>
          ...
          />
          <p className="text-[10.5px] text-gray-400 mt-1">
            Short label shown in the top navigation bar
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-[10.5px] text-gray-400 mt-1">
            Pilih dari daftar atau ketik nama section baru. Pages dengan section
            sama akan dikelompokkan di submenu navbar.
          </p>
        </div>
        ...
        <div>
          ...
          />
          <p className="text-[10.5px] text-gray-400 mt-1">
            Urutan tampil di submenu (1 = paling atas). Kosongkan = urut
            alphabet.
          </p>
        </div>
        ...
        <div className="md:col-span-2">
          ...
          <p className="text-[10.5px] text-gray-500 mt-2">
            Wajib pilih salah satu sebelum Save. Hanya{" "}
            <strong>Published (live)</strong> yang tampil di situs publik.
          </p>
        </div>
          ...
          <div className="md:col-span-2 bg-gray-50 border border-gray-200 p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] text-gray-500 mt-0.5">
                ...
                <LocaleBadge locale={page.linkedIdRecord.locale} /> version:
      ...
      <div className="mt-6">
              ...
              <button
                ...
                onClick={() => {
                  const defaults = {
                    ...
                    text: { type: t as "text", content: "" },
                    stats: {
                      type: t as "stats",
                      items: [{ label: "Label", value: "Value" }],
                    },
                    chart: { type: t as "chart", datasetId: "" },
                    about: {
                      type: t as "about",
                  ...
                  patch({ content: [...(draft.content ?? []), defaults[t]] });
                }}
        ...
        {(draft.content ?? []).length === 0 ? (
          <div className="text-center text-gray-400 text-[12.5px] py-10 border border-dashed border-[#E5E7EB]">
            No content blocks yet. Click + hero / + text / + stats / + chart / +
            about above to add.
          </div>
        ) : (
          <div className="space-y-3">
            {(draft.content ?? []).map((block: any, i: number) => (
              <div key={i} className="bg-white border border-[#E5E7EB] p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[11px] font-semibold text-gray-500 uppercase bg-gray-100 px-2 py-0.5">
                    {block.type}
                  </span>
                  <button
                    type="button"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          c[i] = { ...c[i], headline: e.target.value };
                          patch({ content: c });
                        }}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const c = [...(draft.content ?? [])];
                          c[i] = { ...c[i], subheadline: e.target.value };
                          patch({ content: c });
                        }}
                        className="w-full border border-[#E5E7EB] px-2 py-1.5 text-[12.5px] focus:outline-none focus:border-gray-900"
                      />
                ...
                {block.type === "text" && (
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => {
                        const c = [...(draft.content ?? [])];
                        c[i] = { ...c[i], content: e.target.value };
                        patch({ content: c });
                      }}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => {
                        try {
                          const c = [...(draft.content ?? [])];
                          c[i] = { ...c[i], items: JSON.parse(e.target.value) };
                          patch({ content: c });
                        } catch {}
                      }}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const c = [...(draft.content ?? [])];
                          c[i] = { ...c[i], datasetId: e.target.value };
                          patch({ content: c });
                        }}
                        className="w-full border border-[#E5E7EB] px-2 py-1.5 text-[12.5px] focus:outline-none focus:border-gray-900"
                      />
      ...
      {saveError && (
        <div className="mb-4 text-[12.5px] text-red-600">{saveError}</div>
      )}
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <button
          ...
          title={
            !statusOk ? "Pilih Draft atau Published terlebih dahulu" : undefined
          }
        ...
        {(updateMut.isSuccess || createMut.isSuccess) && !savedSlug && (
          <span className="text-[12px] text-green-600 ml-2 flex items-center gap-1">
      ...
      {savedSlug && (
        <div className="mt-4 bg-gray-50 border border-gray-200 p-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            >
              View on site → {window.location.origin}
              {viewHref}
            </a>
            {draft.status === "draft" && (
              <span className="text-[11px] text-gray-500">
                Draft tidak tampil di situs sampai Anda pilih Published lalu
                Save lagi.
              </span>
            )}
            <span className="text-gray-300">·</span>
...
function PostEditor({
  ...
  const doSave = (onSuccess?: () => void) => {
    ...
    if (isNew) {
      createMut.mutate(
        payload as Omit<BlogPost, "id" | "createdAt" | "updatedAt">,
        {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <LocaleBadge
            locale={(draft.locale ?? post?.locale ?? "en") as "en" | "id"}
          />
        </div>
        {/* Auto-save status indicator */}
        <div className="flex items-center gap-2">
      ...
      <div className="bg-white border border-[#E5E7EB] p-6 grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
        ...
        <div>
          ...
          <p className="text-[10.5px] text-gray-400 mt-1">
            Filtering tag for sub-menu placement.
          </p>
        </div>
        <div>
          <label className="block text-[11.5px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
        ...
        <div>
          ...
          <p className="text-[10.5px] text-gray-500 mt-2">
            Wajib pilih salah satu sebelum Save. Hanya{" "}
            <strong>Published (live)</strong> yang tampil di situs.
          </p>
        </div>
        ...
        <div className="mt-4 bg-gray-50 border border-gray-200 p-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <a
              ...
              className="text-[12.5px] font-medium text-gray-900 underline"
            >
              View article → {window.location.origin}
              {articleHref}
            </a>
            {draft.status === "draft" && (
              <span className="text-[11px] text-gray-500">
...
function FeaturedInsightsTab() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useFeaturedInsights(activeLocale);
  ...
  const handleReset = () => {
    ...
    resetMut.mutate(undefined, {
      onSuccess: () => setActiveLocale(activeLocale === "en" ? "id" : "en"),
    });
  };
  ...
  const addSlug = (slug: string) => {
    const order =
      draft.slugs.length > 0
        ? Math.max(...draft.slugs.map((s) => s.order)) + 1
        : 1;
  ...
  const patchMeta = (
    field: keyof Pick<
      ...
      "title" | "subtitle" | "sectionLabel" | "limit"
    >,
    value: string | number,
  ) => {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[12px] text-gray-400 mt-0.5">
            Configure which research posts appear on the homepage — order,
            title, subtitle
          </p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-5">
          <div className="bg-white border border-[#E5E7EB] rounded-lg p-5 space-y-4">
            ...
            {(["sectionLabel", "title", "subtitle"] as const).map((field) => (
              <div key={field}>
                ...
                <input
                  value={draft[field]}
                  onChange={(e) => patchMeta(field, e.target.value)}
          ...
          {updateMut.isSuccess && (
            <div className="text-[12px] text-gray-500">Saved successfully</div>
          )}
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white border border-[#E5E7EB] rounded-lg p-5">
            ...
            <p className="text-[11px] text-gray-400 mb-4">
              Order 1 = hero card (spans 2 cols) · {draft.slugs.length}/
              {draft.limit} slots used
            </p>
              ...
              <div className="space-y-2">
                {orderedSlugs.map((item, idx) => {
                  ...
                  return (
                    <div
                      key={item.slug}
                      className="flex items-center gap-3 border border-gray-200 rounded px-3 py-2.5 hover:border-gray-300 transition-colors"
                    >
                      <div
                      ...
                      <div className="flex-1 min-w-0">
                        {post ? (
                          ...
                          <div>
                            <div className="text-[12.5px] font-medium text-gray-600 truncate">
                              {item.slug
                                .replace(/-/g, " ")
                                .replace(/\b\w/g, (c) => c.toUpperCase())}
                            </div>
                            <div className="text-[11px] text-amber-600">
                              Slug: {item.slug} (post not found in published
                              posts)
                            </div>
                          </div>
                        )}
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          onClick={() => moveSlug(item.slug, 1)}
                          disabled={idx === orderedSlugs.length - 1}
                          className="p-1 text-gray-400 hover:text-gray-700 disabled:opacity-30 disabled:cursor-not-allowed"
                          title="Move down"
                        >
          ...
          <div className="bg-white border border-[#E5E7EB] rounded-lg p-5">
            ...
            {availablePosts.length === 0 ? (
              <p className="text-[12px] text-gray-400">
                All published posts are already selected.
              </p>
            ) : (
              <div className="flex flex-wrap gap-2">
...
function DataHubTab() {
    ...
    if (ext === "csv" || ext === "tsv" || ext === "txt") {
      ...
      reader.onload = (e) => {
        try {
          ...
          const lines = nonEmptyRaw.filter((l) =>
            parse(l).some((c) => c.trim() !== ""),
          );
          if (lines.length < 2) {
          ...
          while (hi < lines.length && !parse(lines[hi]).some((c) => c.trim()))
            hi++;
          ...
          headers = headers.map((h) => {
            const k = h || "_";
            if (seenH[k] === undefined) {
              seenH[k] = 0;
              return h || `col_${Object.keys(seenH).length}`;
            }
            seenH[k]++;
            return h
              ? `${h}_${seenH[k]}`
              : `col_${Object.keys(seenH).length}_${seenH[k]}`;
          });
          ...
          const primaryCol =
            labelCols.find((h) =>
              allRows.some((r) => (r[headers.indexOf(h)] ?? "").length > 5),
            ) ??
            labelCols[0] ??
            headers[0];

          const valueCols = (
            numericCols.length > 0 ? numericCols : headers
          ...
          const chartType: "line" | "bar" | "area" =
            valueCols.length === 1 && allRows.length <= 12
              ? "bar"
              : valueCols.length === 1
                ? "area"
                : "line";
          ...
          const headersLookLikeTime =
            headers.slice(1).filter((h) => looksLikeTimePeriod(h)).length >
            (headers.length - 1) * 0.5;
          const firstColLooksLikeNames = allRows
            .slice(0, 5)
            .some((r) => (r[0] ?? "").length > 5);

          if (headersLookLikeTime && firstColLooksLikeNames) {
            console.log(
              "Auto-transposing: Headers look like time periods and first column looks like category names.",
            );
            // Perform transpose
            ...
            initialValueCols = (numC.length > 0 ? numC : initialHeaders).filter(
              (c) => c !== initialPrimaryCol,
            );
            const pIdxNew = initialHeaders.indexOf(initialPrimaryCol);
            initialChartData = initialRows.map((row) => {
              const item: Record<string, string | number> = {
                [initialPrimaryCol]: row[pIdxNew] ?? "",
              };
              initialValueCols.forEach((col) => {
                item[col] = parseNumericCell(
                  row[initialHeaders.indexOf(col)] ?? "",
                );
              });
              return item;
            });
  ...
  const handleReset = () => {
    if (!confirm("Reset all datasets to seed state? All edits will be lost."))
      return;
    resetMut.mutate(undefined, {
      onSuccess: () => {
        setSelected(null);
  ...
  const createFromCsvPreview = async () => {
    ...
    try {
      ...
      setCsvPreview(null);
      alert(
        `Dataset "${datasetTitle}" created successfully with ${dataset.rows.length} rows!`,
      );
    } catch (err: any) {
  ...
  const toggleTranspose = () => {
    ...
    const valueCols = (
      numericCols.length > 0 ? numericCols : newHeaders
    ).filter((c) => c !== primaryCol);
    ...
    setCsvPreview({
      ...
      suggestedChart: {
        ...
        rows: chartData,
        colors: valueCols.map(
          (_: string, i: number) => CHART_PALETTE[i % CHART_PALETTE.length],
        ),
      },
    });
  };
                        ...
                        {csvPreview.tableData.slice(0, 100).map((row, i) => (
                          ...
                          >
                            {csvPreview.headers.map((h, j) => (
                              <td
                                key={j}
                                className="border border-gray-200 px-3 py-1.5 text-gray-800 whitespace-nowrap"
                              >
        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-[12px] text-gray-400 mt-0.5">
                {isLoading
                  ? "Loading…"
                  : `${datasets.length} datasets — served from API`}
              </p>
            </div>
            <div className="flex items-center gap-2">
              <button
          ...
          {isLoading ? (
            <div className="flex items-center justify-center py-24 gap-3 text-gray-400">
              <Loader2 className="w-5 h-5 animate-spin" />{" "}
              <span className="text-[13.5px]">Fetching from API…</span>
            </div>
          ) : (
            <div className="bg-white border border-[#E5E7EB]">
              ...
              {datasets.length === 0 ? (
                ...
                datasets.map((ds) => (
                  <div
                    key={ds.id}
                    className="grid grid-cols-12 border-b border-[#F3F4F6] px-4 py-3.5 items-center hover:bg-gray-50"
                  >
                    <div className="col-span-4">
                      ...
                      <div className="text-[11.5px] text-gray-400 mt-0.5">
                        {ds.description?.slice(0, 55)}
                        {ds.description?.length > 55 ? "…" : ""}
                      </div>
                    </div>
                    ...
                    <div className="col-span-2">
                      <span className="text-[11.5px] font-medium text-gray-700 bg-gray-100 px-2 py-0.5 capitalize">
                        {ds.chartType}
                      </span>
                    </div>
                    <div className="col-span-2 text-[12.5px] text-gray-600">
          ...
          <div className="mt-5 bg-gray-50 border border-gray-200 p-4">
            <p className="text-[12px] text-gray-600 leading-relaxed">
              All data is served via the <strong>REST API</strong> backed by an
              in-memory store. Click "Reset to Seed" to restore original
              datasets. In production, replace the store with PostgreSQL +
              Drizzle ORM.
            </p>
          </div>
        </div>
    ...
    <div>
      {deleteSuccess && (
        <div className="mb-4 bg-gray-50 border border-gray-200 p-3 flex items-center justify-between">
          ...
          <a href="/admin" className="text-[12px] text-gray-900 underline">
            Refresh list
          </a>
        </div>
      )}
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[12px] text-gray-400 mt-0.5">
            {isLoading
              ? "Loading…"
              : `${pages.length} page versions across ${Object.keys(grouped).length} pages`}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
                ...
                <div key={slug}>
                  ...
                  <div className="grid grid-cols-12 border-b border-[#F3F4F6] px-4 py-3 items-center hover:bg-gray-50">
                    <div className="col-span-1">
                      <button
                        onClick={() =>
                          setExpandedId(isExpanded ? null : anyPage.id)
                        }
                    ...
                    <div className="col-span-3 text-[12px] text-gray-500 font-mono truncate">
                      {slug}
                    </div>
                    ...
                    <div className="col-span-2 flex items-center gap-2">
                      <button
                        onClick={() => openEdit(anyPage)}
                        className="text-[12px] font-medium text-gray-900 hover:underline"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => handleDelete(anyPage.id, anyPage.title)}
                  ...
                  {isExpanded &&
                    pgList.map((p) => (
                      <div
                        key={p.id}
                        className="grid grid-cols-12 border-b border-[#F3F4F6] px-4 py-2.5 bg-gray-50/50 items-center"
                      >
                        <div className="col-span-1" />
                        ...
                        <div className="col-span-3 text-[12px] text-gray-400 font-mono pl-4">
                          {p.slug}
                        </div>
                        <div className="col-span-2">
                          <LocaleBadge locale={p.locale} />
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[12px] text-gray-400 mt-0.5">
            {isLoading
              ? "Loading…"
              : `${posts.length} post versions across ${Object.keys(grouped).length} posts`}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        {[
          ["all", "All"],
          ...BLOG_CATEGORIES.map((c) => [c, c] as [string, string]),
        ].map(([v, label]) => (
          <button
            key={v}
                ...
                <div key={slug}>
                  <div className="grid grid-cols-12 border-b border-[#F3F4F6] px-4 py-3 items-center hover:bg-gray-50">
                    <div className="col-span-1">
                      <button
                        onClick={() =>
                          setExpandedId(isExpanded ? null : anyPost.id)
                        }
                        className="text-gray-400 hover:text-gray-700"
                      >
                        {isExpanded ? (
                    ...
                    <div className="col-span-3">
                      <div className="flex items-center gap-2">
                        {postList.find((p) => p.locale === "en") && (
                          <LocaleBadge locale="en" />
                        )}
                        {postList.find((p) => p.locale === "id") && (
                          <>
                            <span className="text-gray-300">·</span>
                            <LocaleBadge locale="id" />
                          </>
                    ...
                    <div className="col-span-1">
                      <StatusBadge status={anyPost.status} />
                    </div>
                    ...
                    <div className="col-span-2 flex items-center gap-2">
                      ...
                      <button
                        onClick={() => handleDelete(anyPost.id, anyPost.title)}
                        className="text-gray-400 hover:text-red-500"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
...
function newWidget(type: AnalysisWidgetType): AnalysisWidget {
  ...
  if (type === "comparison")
    return {
      ...base,
      compareHeaders: ["Item", "Value 1", "Value 2"],
      compareItems: [],
    };
  if (type === "highlight")
    return { ...base, calloutColor: "#1a3a5c", text: "" };
...
function WidgetEditor({
  ...
  const patch = (fields: Partial<AnalysisWidget>) =>
    onChange({ ...widget, ...fields });

  const addMetric = () =>
    patch({ metrics: [...(widget.metrics ?? []), newMetric()] });
  const removeMetric = (mid: string) =>
    patch({ metrics: widget.metrics?.filter((m) => m.id !== mid) });
  ...
  const addDistItem = () =>
    patch({
      distributionItems: [
        ...(widget.distributionItems ?? []),
        { label: "New Item", value: 0, percentage: 0, color: "#1a3a5c" },
      ],
    });
  const removeDistItem = (idx: number) =>
    patch({
      distributionItems: widget.distributionItems?.filter((_, i) => i !== idx),
    });
  type DistItem = {
    label: string;
    value: number;
  ...
  const updateDistItem = (idx: number, fields: Partial<DistItem>) =>
    patch({
      distributionItems: widget.distributionItems?.map((d, i) =>
        i === idx ? { ...d, ...fields } : d,
      ),
    });
  ...
  const updateCompareRow = (idx: number, fields: Partial<CompareItem>) =>
    patch({
      compareItems: widget.compareItems?.map((c, i) =>
        i === idx ? { ...c, ...fields } : c,
      ),
    });
  ...
  const addBarItem = () =>
    patch({
      barData: [
        ...(widget.barData ?? []),
        { label: "Item", value: 0, color: "#1a3a5c" },
      ],
    });
  ...
  const updateBarItem = (idx: number, fields: Partial<BarItem>) =>
    patch({
      barData: widget.barData?.map((b, i) =>
        i === idx ? { ...b, ...fields } : b,
      ),
    });
      ...
      {widget.type === "metric-card" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={addMetric}
              className="text-[11px] text-gray-900 font-medium hover:underline flex items-center gap-1"
            >
          ...
          {(widget.metrics ?? []).map((m) => (
            <div
              key={m.id}
              className="bg-white border border-gray-200 rounded p-3 grid grid-cols-12 gap-2 items-end"
            >
              <div className="col-span-3">
                ...
                <input
                  value={m.label}
                  onChange={(e) =>
                    updateMetric(m.id, { label: e.target.value })
                  }
              ...
              <div className="col-span-2">
                ...
                <input
                  value={m.trendValue ?? ""}
                  onChange={(e) =>
                    updateMetric(m.id, { trendValue: e.target.value })
                  }
                  className="w-full border border-gray-300 px-2 py-1 text-[12px] focus:outline-none focus:border-gray-900"
                  placeholder="+2.3%"
                />
              ...
              <div className="col-span-12">
                ...
                <input
                  value={m.note ?? ""}
                  onChange={(e) => updateMetric(m.id, { note: e.target.value })}
                  className="w-full border border-gray-300 px-2 py-1 text-[12px] focus:outline-none focus:border-gray-900"
      ...
      {widget.type === "distribution" && (
        <div className="space-y-3">
          ...
          {(widget.distributionItems ?? []).map((d, i) => (
            <div
              key={i}
              className="bg-white border border-gray-200 rounded p-3 grid grid-cols-12 gap-2 items-center"
            >
              <div className="col-span-1">
                <input
                  type="color"
                  value={d.color ?? "#1a3a5c"}
                  onChange={(e) => updateDistItem(i, { color: e.target.value })}
              ...
              <div className="col-span-4">
                <input
                  value={d.label}
                  onChange={(e) => updateDistItem(i, { label: e.target.value })}
                  className="w-full border border-gray-300 px-2 py-1 text-[12px] focus:outline-none focus:border-gray-900"
                />
              </div>
              <div className="col-span-2">
                <input
                  ...
                  value={d.value}
                  onChange={(e) =>
                    updateDistItem(i, { value: Number(e.target.value) })
                  }
                  className="w-full border border-gray-300 px-2 py-1 text-[12px] focus:outline-none focus:border-gray-900"
                />
              </div>
              <div className="col-span-2">
                <div className="flex items-center gap-1">
                  <input
                    ...
                    value={d.percentage}
                    onChange={(e) =>
                      updateDistItem(i, { percentage: Number(e.target.value) })
                    }
                    className="w-full border border-gray-300 px-2 py-1 text-[12px] focus:outline-none focus:border-gray-900"
                  />
      ...
      {widget.type === "comparison" && (
        <div className="space-y-3">
          <div>
            ...
            <input
              value={(widget.compareHeaders ?? []).join(", ")}
              onChange={(e) =>
                patch({
                  compareHeaders: e.target.value
                    .split(",")
                    .map((s) => s.trim())
                    .filter(Boolean),
                })
          ...
          {(widget.compareItems ?? []).map((c, i) => (
            <div
              key={i}
              className="bg-white border border-gray-200 rounded p-3 flex gap-2 items-center"
            >
              <div className="flex-1">
                <input
                  value={c.label}
                  onChange={(e) =>
                    updateCompareRow(i, { label: e.target.value })
                  }
                  className="w-full border border-gray-300 px-2 py-1 text-[12px] focus:outline-none focus:border-gray-900"
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <input
                type="color"
                value={widget.calloutColor ?? "#1a3a5c"}
          ...
          <div>
            <label className="block text-[10px] text-gray-500 mb-1">
              Callout Text (supports markdown-style bold **text**)
            </label>
            <textarea
              rows={4}
              value={widget.text ?? ""}
          ...
          {(widget.barData ?? []).map((b, i) => (
            ...
            >
              <div className="col-span-1">
                <input
                  type="color"
                  value={b.color ?? "#1a3a5c"}
                  onChange={(e) => updateBarItem(i, { color: e.target.value })}
                  className="w-8 h-8 rounded border border-gray-300 cursor-pointer"
                />
              </div>
              <div className="col-span-6">
                <input
                  value={b.label}
                  onChange={(e) => updateBarItem(i, { label: e.target.value })}
                  className="w-full border border-gray-300 px-2 py-1 text-[12px] focus:outline-none focus:border-gray-900"
                  placeholder="Label"
                />
              </div>
              <div className="col-span-4">
                <input
                  ...
                  value={b.value}
                  onChange={(e) =>
                    updateBarItem(i, { value: Number(e.target.value) })
                  }
                  className="w-full border border-gray-300 px-2 py-1 text-[12px] focus:outline-none focus:border-gray-900"
                  placeholder="Value"
                />
...
function SectionEditor({
  ...
  const patch = (fields: Partial<AnalysisSection>) =>
  ...
  const moveWidget = (idx: number, dir: -1 | 1) => {
    ...
    [newWidgets[idx], newWidgets[target]] = [
      newWidgets[target],
      newWidgets[idx],
    ];
      ...
      <div className="bg-gray-50 border-b border-gray-200 px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            onChange={(e) =>
              patch({
                sectionType: e.target.value as AnalysisSection["sectionType"],
              })
            }
...
function AnalisisEditor({
  ...
  const patch = (fields: Partial<AnalisisDeskriptif>) =>
    setDraft((prev) => ({ ...prev, ...fields }));

  const addSection = () => {
    const currentSections = draft.sections ?? [];
    patch({
      sections: [...currentSections, newSection(currentSections.length + 1)],
    });
  };

  const updateSection = (id: string, s: AnalysisSection) => {
    const updated = (draft.sections ?? []).map((section) =>
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-[12px] text-gray-400 mt-0.5">
                ...
                {(draft.sections ?? []).length !== 1 ? "s" : ""} — Drag to
                reorder widgets within each section
              </p>
            </div>
            <button
              onClick={addSection}
        ...
        {/* Save */}
        {saveError && (
          <div className="text-[12.5px] text-red-600">{saveError}</div>
        )}
...
function AnalisisTab() {
  ...
  const handleSave = (data: Partial<AnalisisDeskriptif>) => {
    if (isNew) {
      createMut.mutate(
        data as Omit<AnalisisDeskriptif, "id" | "createdAt" | "updatedAt">,
        {
          onSuccess: () => {
            setIsNew(false);
            setEditRecord(null);
  ...
  const handleReset = () => {
    if (
      !confirm(
        "Reset all analysis to seed state? All custom edits will be lost.",
      )
    )
      return;
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[12px] text-gray-400 mt-0.5">
            {isLoading
              ? "Loading…"
              : `${records.length} analysis record${records.length !== 1 ? "s" : ""} — customizable descriptive analysis sections`}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
                  ...
                  {(r.sections ?? [])
                    ...
                    .map((s) => (
                      ...
                      >
                        ...
                        <span className="text-[10px] text-gray-400">
                          ({(s.widgets ?? []).length} widget
                          {(s.widgets ?? []).length !== 1 ? "s" : ""})
                        </span>
      ...
      <div className="mt-6">
        <p className="text-[12.5px] font-semibold text-gray-500 mb-1">
          Customizable Descriptive Analysis
        </p>
        <p className="text-[12px] text-gray-700 leading-relaxed">
          Each analysis record supports multiple sections. Each section can
          contain multiple widgets:
          <strong> Metric Cards</strong> (KPI with trends),{" "}
          <strong>Distribution Charts</strong> (donut/bar),
          <strong> Comparison Tables</strong>,{" "}
          <strong>Highlight Callouts</strong>, <strong>Bar Charts</strong>, and{" "}
          <strong>Custom Text</strong>. Edit widgets inline, reorder with ↑↓
          buttons, and preview changes live.
        </p>
      </div>
    </div>
  );
...
function newExchangeRate(
  order: number,
): Omit<ExchangeRate, "id" | "createdAt" | "updatedAt"> {
  return {
    symbol: "NEW/USD",
    label: "NEW/USD",
    labelEn: "NEW/USD",
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[12px] text-gray-500 mt-1">
            Manage exchange rates displayed in the market ticker and data hub.
          </p>
        </div>
        <div className="flex gap-2">
          <button
        ...
        <div className="space-y-3">
          ...
          {allItems.map(({ id, isNew }) => {
            ...
            return (
              <div
                key={id}
                className={`border p-5 ${isEditing ? "border-gray-900 bg-white" : "border-[#E5E7EB] bg-white"}`}
              >
                <div className="flex items-start gap-4 flex-wrap">
                  {/* Symbol + Category */}
                  <div className="min-w-[120px]">
                    <label className="block text-[10.5px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
                      Symbol
                    </label>
                    <input
                      type="text"
                  ...
                  <div className="min-w-[140px]">
                    ...
                    <input
                      ...
                      value={d.labelEn ?? ""}
                      onChange={(e) =>
                        patchDraft(id, { labelEn: e.target.value })
                      }
                  ...
                  <div className="min-w-[140px]">
                    ...
                    <input
                      ...
                      value={d.labelId ?? ""}
                      onChange={(e) =>
                        patchDraft(id, { labelId: e.target.value })
                      }
                      className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900"
                  ...
                  <div className="min-w-[100px]">
                    ...
                    <input
                      ...
                      value={d.value ?? ""}
                      onChange={(e) =>
                        patchDraft(id, { value: e.target.value })
                      }
                      className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[13px] font-medium text-gray-900 focus:outline-none focus:border-gray-900"
                  ...
                  <div className="min-w-[100px]">
                    ...
                    <input
                      ...
                      value={d.change ?? ""}
                      onChange={(e) =>
                        patchDraft(id, { change: e.target.value })
                      }
                      className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900"
                      placeholder="+0.32%"
                    />
                    ...
                    <div className="flex flex-col gap-1.5">
                      <select
                        ...
                        onChange={(e) => {
                          const val = e.target.value;
                          if (val !== "custom")
                            patchDraft(id, { category: val as any });
                      ...
                      {!CATEGORIES_ER.includes(d.category ?? "") && (
                        <input
                          ...
                          value={d.category ?? ""}
                          onChange={(e) =>
                            patchDraft(id, { category: e.target.value as any })
                          }
                          className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900 bg-white"
                  ...
                  <div className="flex items-center gap-2 pt-5">
                    ...
                    >
                      <span
                        className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-transform ${d.enabled ? "left-5" : "left-0.5"}`}
                      />
                    </button>
                    <span className="text-[11px] text-gray-500">
...
function CalendarTab() {
  ...
  const [configDraft, setConfigDraft] =
    useState<Partial<CalendarConfig> | null>(null);
  const [newCount, setNewCount] = useState(0);
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[12px] text-gray-500 mt-1">
            Manage economic events for display. Changes appear in the calendar
            widget on pages.
          </p>
        </div>
        <div className="flex gap-2">
      ...
      {configTab === "settings" && (
        <div className="max-w-[700px]">
          {activeConfig && (
            <div className="bg-white border border-[#E5E7EB] p-6">
              <h3 className="text-[14px] font-semibold text-gray-900 mb-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={activeConfig.titleId ?? ""}
                    onChange={(e) =>
                      setConfigDraft((p) =>
                        p ? { ...p, titleId: e.target.value } : p,
                      )
                    }
                    className="w-full border border-[#E5E7EB] px-3 py-2 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    min={1}
                    max={30}
                    value={activeConfig.defaultDays}
                    onChange={(e) =>
                      setConfigDraft((p) =>
                        p
                          ? { ...p, defaultDays: parseInt(e.target.value) || 7 }
                          : p,
                      )
                    }
                    className="w-full border border-[#E5E7EB] px-3 py-2 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900"
                ...
                <div>
                  ...
                  <select
                    value={activeConfig.regionFilter}
                    onChange={(e) =>
                      setConfigDraft((p) =>
                        p
                          ? {
                              ...p,
                              regionFilter: e.target.value as CalendarRegion,
                            }
                          : p,
                      )
                    }
                  ...
                  {(
                    ...
                    <div key={key} className="flex items-center gap-2">
                      <button
                        onClick={() =>
                          setConfigDraft((p) =>
                            p ? { ...p, [key]: !(p as any)[key] } : p,
                          )
                        }
                        className={`w-10 h-5 rounded-full transition-colors relative ${(activeConfig as any)[key] ? "bg-gray-900" : "bg-gray-300"}`}
                      >
                        <span
                          className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-transform ${(activeConfig as any)[key] ? "left-5" : "left-0.5"}`}
                        />
                      </button>
                      <span className="text-[12.5px] text-gray-700">
                        {label}
              ...
              <div className="mt-5 pt-5 border-t border-[#E5E7EB]">
                ...
                <div className="flex gap-3">
                  {(["low", "medium", "high"] as CalendarImpact[]).map(
                    (impact) => {
                      const active = (activeConfig.impactFilter ?? []).includes(
                        impact,
                      );
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            const next = active
                              ? curr.filter((i) => i !== impact)
                              : [...curr, impact];
                            setConfigDraft((p) =>
                              p ? { ...p, impactFilter: next } : p,
                            );
                          }}
                        ...
                        >
                          <span
                            className={`w-2 h-2 rounded-full ${impact === "low" ? "bg-gray-300" : impact === "medium" ? "bg-yellow-400" : "bg-red-500"}`}
                          />
                          {impact}
                        </button>
                      );
              ...
              <div className="mt-5 flex gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 text-[12.5px] font-medium text-gray-600 border border-[#E5E7EB] hover:bg-gray-50"
                >
                  Cancel
                </button>
              </div>
                    ...
                    <div className="flex items-start gap-3 flex-wrap">
                      ...
                      <div className="min-w-[110px]">
                        ...
                        <input
                          ...
                          value={d.date ?? ""}
                          onChange={(e) =>
                            patchDraft(id, { date: e.target.value })
                          }
                          className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900"
                        />
                      </div>
                      ...
                      <div className="min-w-[90px]">
                        ...
                        <input
                          ...
                          value={d.time ?? "08:00"}
                          onChange={(e) =>
                            patchDraft(id, { time: e.target.value })
                          }
                          className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900"
                        />
                      </div>
                      ...
                      <div className="min-w-[130px]">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            patchDraft(id, { countryLabel: e.target.value })
                          }
                          className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900"
                      ...
                      <div className="min-w-[200px]">
                        ...
                        <input
                          ...
                          value={d.eventName ?? ""}
                          onChange={(e) =>
                            patchDraft(id, { eventName: e.target.value })
                          }
                          className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900"
                          placeholder="CPI YoY"
                        />
                      ...
                      <div className="min-w-[200px]">
                        ...
                        <input
                          ...
                          value={d.eventNameId ?? ""}
                          onChange={(e) =>
                            patchDraft(id, { eventNameId: e.target.value })
                          }
                          className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900"
                          placeholder="IHK YoY"
                        />
                        ...
                        <div className="flex flex-col gap-1.5">
                          ...
                          {!CALENDAR_CATEGORIES.includes(d.category ?? "") && (
                            <input
                              ...
                              value={d.category ?? ""}
                              onChange={(e) =>
                                patchDraft(id, { category: e.target.value })
                              }
                              className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[12.5px] text-gray-900 focus:outline-none focus:border-gray-900 bg-white"
                              placeholder="Type custom category..."
                            />
                      ...
                      <div className="min-w-[100px]">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            patchDraft(id, {
                              impact: e.target.value as CalendarImpact,
                            })
                          }
                      ...
                      <div className="min-w-[100px]">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            patchDraft(id, {
                              region: e.target.value as CalendarRegion,
                            })
                          }
                          className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[12.5px] text-gray-900 focus:outline-none focus:border-gray-900 bg-white"
                        >
                    ...
                    <div className="flex items-start gap-3 mt-3 flex-wrap">
                      <div className="min-w-[90px]">
                        ...
                        <input
                          ...
                          value={d.actual ?? ""}
                          onChange={(e) =>
                            patchDraft(id, { actual: e.target.value })
                          }
                          className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900"
                          placeholder="2.1%"
                        />
                      ...
                      <div className="min-w-[90px]">
                        ...
                        <input
                          ...
                          value={d.previous ?? ""}
                          onChange={(e) =>
                            patchDraft(id, { previous: e.target.value })
                          }
                          className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900"
                          placeholder="1.8%"
                        />
                      ...
                      <div className="min-w-[90px]">
                        ...
                        <input
                          ...
                          value={d.consensus ?? ""}
                          onChange={(e) =>
                            patchDraft(id, { consensus: e.target.value })
                          }
                          className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900"
                          placeholder="2.0%"
                        />
                      ...
                      <div className="min-w-[90px]">
                        ...
                        <input
                          ...
                          value={d.forecast ?? ""}
                          onChange={(e) =>
                            patchDraft(id, { forecast: e.target.value })
                          }
                          className="w-full border border-[#E5E7EB] px-2.5 py-1.5 text-[13px] text-gray-900 focus:outline-none focus:border-gray-900"
                          placeholder="2.2%"
                        />
                      ...
                      <div className="flex items-center gap-2 pt-4">
                        <button
                          onClick={() =>
                            patchDraft(id, { enabled: !d.enabled })
                          }
                          className={`w-10 h-5 rounded-full transition-colors relative ${d.enabled ? "bg-gray-900" : "bg-gray-300"}`}
                        >
                          <span
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[12px] text-gray-400 mt-0.5">
            {isLoading
              ? "Loading…"
              : `${entries.length} events — auto-refresh every 5s`}
            {lastUpdate && (
        ...
        <button
          onClick={() => {
            if (confirm("Clear all activity logs?")) clearMut.mutate();
          }}
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center border-2 border-dashed border-gray-200 rounded-lg">
          ...
          <p className="text-[14px] text-gray-400">
            No activity yet. Changes made in the CMS will appear here.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-[12.5px]">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                <th className="text-left px-4 py-2.5 font-semibold text-gray-500 uppercase tracking-wide text-[11px] w-[110px]">
                  Time
                </th>
                <th className="text-left px-4 py-2.5 font-semibold text-gray-500 uppercase tracking-wide text-[11px] w-[90px]">
                  Action
                </th>
                <th className="text-left px-4 py-2.5 font-semibold text-gray-500 uppercase tracking-wide text-[11px] w-[100px]">
                  Resource
                </th>
                <th className="text-left px-4 py-2.5 font-semibold text-gray-500 uppercase tracking-wide text-[11px]">
                  Detail
                </th>
                <th className="text-left px-4 py-2.5 font-semibold text-gray-500 uppercase tracking-wide text-[11px] w-[120px]">
                  IP
                </th>
              </tr>
            </thead>
            <tbody>
              {entries.map((e: ActivityLogEntry, i: number) => (
                <tr
                  key={e.id}
                  className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${i === 0 ? "bg-green-50/40" : ""}`}
                >
                  <td
                    className="px-4 py-2.5 text-gray-400 whitespace-nowrap"
                  ...
                  <td className="px-4 py-2.5">
                    <span
                      className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${ACTION_COLORS[e.action] ?? "bg-gray-100 text-gray-600"}`}
                    >
                      {e.action}
                    </span>
                  </td>
                  ...
                  <td
                    className="px-4 py-2.5 text-gray-700 max-w-[400px] truncate"
                    title={e.detail}
                  >
                    {e.detail ?? "—"}
                  </td>
    ...
    <div className="min-h-screen bg-[#F9FAFB]">
      ...
      <div className="bg-white border-b border-[#E5E7EB] px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <a
            href="/"
            className="text-[12.5px] text-gray-500 hover:text-gray-800 font-medium"
          >
            ← Back to site
          </a>
        </div>
