...
function formatChange(
  ...
  const sign = diff > 0 ? "+" : "";
  return {
    label: isRate ? `${sign}${diff.toFixed(2)}%` : `${sign}${diff.toFixed(2)}`,
    positive: diff > 0,
  };
}
...
export default function DataHub() {
  ...
  const biRate = biRateDs
    ? (() => {
        ...
        const { label, positive } = formatChange(last, prev, true);
        return {
          label: "BI Rate",
          value: formatValue(
            last,
            biRateDs.unitType ?? "percent",
            biRateDs.unit,
          ),
          change: label,
          positive,
  ...
  const idrUsd = idrUsdDs
    ? (() => {
        ...
        return {
          ...
          value: formatValue(
            last,
            idrUsdDs.unitType ?? "currency_idr",
            idrUsdDs.unit,
          ),
          change: label,
          positive,
  ...
  const jciSpark = idrUsdDs
    ? idrUsdDs.rows
        .map((r) => ({
          t: String(r[idrUsdDs.columns[0]]),
          v:
            typeof r["IDR/USD"] === "number"
              ? (r["IDR/USD"] as number)
              : parseFloat(String(r["IDR/USD"])),
        }))
        .filter((p) => !isNaN(p.v))
    : [];
  ...
  const {
    brushRange: sparkBrush,
    setBrushRange: setSparkBrush,
    zoomProps: sparkZoomProps,
  } = useChartZoom(jciSpark.length);
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-[13px] text-gray-400 mt-0.5">
              {t("data_hub_latest_desc")}
            </p>
          </div>
          <Link
            href="/data"
            className="flex items-center gap-1 text-[12.5px] font-medium text-gray-900 hover:underline"
          >
            {t("explore_full_data_hub")} <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        </div>
          ...
          <div className="md:col-span-3 border border-[#E5E7EB] bg-white">
            <div className="px-4 py-3 border-b border-[#E5E7EB] flex items-center gap-2">
              ...
              <Link
                ...
                className="text-[11.5px] text-gray-900 font-medium hover:underline ml-auto"
              >
                {t("view_all")} →
              </Link>
            </div>
            {recentPosts.length === 0 ? (
              <div className="px-4 py-8 text-center text-gray-400 text-[13px]">
                {t("no_articles_yet")}{" "}
                <Link href="/admin" className="text-gray-900 hover:underline">
                  {t("add_in_cms")} →
                </Link>
              </div>
            ) : (
                    ...
                    >
                      ...
                      <td className="py-3 pr-2">
                        <Link
                          ...
                          className="text-[12.5px] text-gray-800 font-medium hover:text-gray-900 transition-colors line-clamp-1"
                        >
                          {post.title}
                        </Link>
                      </td>
            ...
            <div className="px-4 py-3 border-t border-[#E5E7EB]">
              ...
              >
                {t("all_research")} <ArrowRight className="w-3.5 h-3.5" />
              </Link>
            </div>
          </div>
          ...
          <div className="md:col-span-2 border border-[#E5E7EB] bg-white flex flex-col">
            ...
            <div className="divide-y divide-[#F3F4F6] flex-1">
              ...
              {datasets
                .filter(
                  (d) =>
                    !["bi-rate", "idr-usd", "trade-balance"].includes(d.id),
                )
                .slice(0, 3)
                .map((ds) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex items-center justify-between px-4 py-2.5 hover:bg-gray-50 transition-colors"
                    >
                      <span className="text-[12.5px] text-gray-600 truncate max-w-[120px]">
                        {ds.title}
                      </span>
                      <div className="flex items-center gap-2">
                        <span className="text-[13px] font-semibold text-gray-900">
