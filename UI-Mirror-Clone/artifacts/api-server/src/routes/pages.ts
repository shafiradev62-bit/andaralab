...
const contentSectionSchema: z.ZodType<
  import("../lib/seed-data.js").ContentSection
> = z.lazy(() =>
...
function problem(
  ...
  res
    .status(status)
    .json({
      type: `https://andarlab.io/errors/${status}`,
      title,
      status,
      detail,
    });
...
router.post("/", (req: Request, res: Response) => {
  ...
  if (!result.success)
    return problem(res, 400, "Validation Error", result.error.message);
...
router.post("/reset", (_req: Request, res: Response) => {
  ...
  res.json({
    data: pageStore.list(),
    meta: { total: pageStore.list().length, reset: true },
  });
});
...
router.post("/link", (req: Request, res: Response) => {
  ...
  if (!idA || !idB)
    return problem(res, 400, "Bad Request", "idA and idB are required");
  if (!pageStore.get(idA) || !pageStore.get(idB))
    return problem(res, 404, "Not Found", "One or both page IDs not found");
  pageStore.linkPages(idA, idB);
  res.json({ data: { idA, idB, linked: true }, meta: { linked: true } });
});

function sendPublicPageBySlug(
  res: Response,
  slugParam: string,
  pathLabel: string,
  locale?: string,
  ...
  if (page) {
    return res.json({
      data: { ...page, linkedIdRecord: pageStore.getLinked(page.id) ?? null },
    });
  }
...
router.get("/lookup", (req: Request, res: Response) => {
  ...
  if (!pathRaw)
    return problem(
      ...
      400,
      "Bad Request",
      "Query parameter 'path' is required (e.g. /macro)",
    );
...
router.get("/:id", (req: Request, res: Response) => {
  ...
  if (id === null)
    return problem(res, 400, "Bad Request", "Page ID must be a number");
  ...
  if (!page)
    return problem(
      ...
      404,
      "Not Found",
      `Page with ID '${id}' does not exist`,
    );
  res.json({
    data: { ...page, linkedIdRecord: pageStore.getLinked(id) ?? null },
  });
});
